...
export function SchoolSwitcher() {
  ...
  <div className="flex-shrink-0 h-12 w-12 rounded-md overflow-hidden bg-sidebar-primary text-sidebar-primary-foreground flex items-center justify-center">
        {(selectedSchool as any).imageUrl ? (
          ...
          <img src={(selectedSchool as any).imageUrl} alt={selectedSchool.name} className="h-full w-full object-cover" />
        ) : (
          <LucideSchool className="h-6 w-6" />
        )}
      </div>
      <div className="flex-1 text-left min-w-0">
        <div className="text-sm font-medium truncate">{selectedSchool.name}</div>
        <div className="text-xs text-muted-foreground truncate">{selectedSchool.type}</div>
      </div>
  ...
  if (user?.role === UserRole.SUPER_ADMIN) {
    return (
      <div className="w-full">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className={`w-full ${isCollapsed ? 'p-2' : 'p-3'} rounded-lg border bg-white hover:shadow-sm`}>
              {isCollapsed ? CompactSchoolContent : SchoolCardContent}
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="min-w-[220px] rounded-lg z-50" align={isCollapsed ? "end" : "start"} sideOffset={8}>
            <DropdownMenuLabel className="text-muted-foreground text-xs">Schools</DropdownMenuLabel>
            {schools.map((school) => (
              <DropdownMenuItem
                key={school._id}
                ...
                <div className="flex items-center gap-2">
                  <div className="h-8 w-8 rounded-md overflow-hidden bg-white flex items-center justify-center border">
                    {(school as any).imageUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={(school as any).imageUrl} alt={school.name} className="h-full w-full object-cover" />
                    ) : (
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem className="gap-2 p-2" onClick={handleCreateSchool}>
              <div className="flex size-6 items-center justify-center rounded-md border bg-transparent">
                <Plus className="size-4" />
              </div>
              <div className="text-muted-foreground font-medium">Add school</div>
  ...
  return (
    <div className="w-full">
      <div className={`w-full ${isCollapsed ? 'p-2' : 'p-3'} rounded-lg border bg-white`}>
        {isCollapsed ? CompactSchoolContent : SchoolCardContent}
      </div>
    </div>
  );
